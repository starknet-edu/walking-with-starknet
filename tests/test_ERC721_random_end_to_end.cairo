...
func test_declare_animals{syscall_ptr: felt*, range_check_ptr}() {
    ...
    let first_animal_token: Uint256 = IERC721Custom.declare_animal(
        contract_address=erc721_custom_address, sex=SEX, legs=n_legs, wings=WINGS
    );
    let second_animal_token: Uint256 = IERC721Custom.declare_animal(
        contract_address=erc721_custom_address, sex=SEX + 1, legs=n_legs - 1, wings=WINGS + 1
    );
    %{ stop_prank_owner() %}
    ...
    let (local sex, legs, wings) = IERC721Custom.get_animal_characteristics(
        contract_address=erc721_custom_address, token_id=one_uint
    );
    assert SEX = sex;
    assert n_legs = legs;
    assert WINGS = wings;
    ...
    let (local sex, legs, wings) = IERC721Custom.get_animal_characteristics(
        contract_address=erc721_custom_address, token_id=two_uint
    );
    assert SEX + 1 = sex;
